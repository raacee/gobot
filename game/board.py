...
from .stones import WHITE_STONE, BLACK_STONE, WHITE_STONE_CHAR, BLACK_STONE_CHAR, EMPTY, EMPTY_CHAR, PASS
...
class Board:
    ...
    def game(self):
        ...
        while True:
            ...
            player_choice = current_player.choose_case(self._board)
            ...
            induces_suicide = self._induces_suicide(x, y, current_player, test_board)

            # Check if case induces capture
            induces_capture = self._induces_capture(x, y, current_player, test_board)
            ...
            for x_n, y_n in neighbors:
                ...
                if neighbor_stone == current_player.stone * -1:
                    ...
                    if 0 not in group_stones:
                        ...
                        print(f"Player {current_player.name} captures group")
    ...
    def _flood_fill(self, x: int, y: int, player: Player) -> set[tuple[int, int]]:
        ...
        while len(queue) > 0:
            ...
            if (x, y) not in res:
                if self._board[x, y] == EMPTY:
                    res.add((x,y))
                    continue
                elif self._board[x, y] == player.stone:
                    res.add((x,y))
                    neighbors = self._neighbors_indices(x, y)
    ...
    def _induces_suicide(self, x: int, y: int, player: Player, board_array: np.ndarray) -> bool:
        """Same algorithm as flood fill, excepts it returns False once it finds a 0"""
        queue = deque()
        queue.append((x, y))
        checked = set()
        while len(queue) > 0:
            ...
            if (x, y) not in checked:
                if board_array[x, y] == EMPTY:
                    checked.add((x,y))
                    return False
                elif board_array[x, y] == player.stone:
                    checked.add((x,y))
                    neighbors = self._neighbors_indices(x, y)
                    queue.extend(neighbors)
        return True

    def _induces_capture(self, x: int, y: int, player: Player, board: np.ndarray) -> bool:
        """Runs flood fill at each neighboring group to detect capture"""
        induces_capture = False
        neighbors = self._neighbors_indices(x, y)
        # already_checked_stones_indices = set()

        # For each neighbor, check if associated group is captured
        for x_n, y_n in neighbors:
            neighbor_stone = board[x_n, y_n]
            if neighbor_stone == player.stone * -1:
                # Gather neigboring group
                group_indices_tuples = self._flood_fill(x_n, y_n, neighbor_stone)
                n_stones = len(group_indices_tuples)
                group_stones = [0] * n_stones
                # Check in group if an empty space is found
                for i, index in enumerate(group_indices_tuples):
                    stone = board[index]
                    group_stones[i] = stone
                    # TODO: check if empty should be checked here
                    # or after whole group gathering
                    # (depends on if the list of checked stone is used or not)
                    # if stone == EMPTY:
                        # already_checked_stones_indices.update(group_stones)
                        # break
                if EMPTY not in group_stones:
                    induces_capture = True
                    print(f"Player {player.name} captures group")
                    if neighbor_stone == BLACK_STONE:
                        self._captured_black_stones += n_stones
                    for i, index in enumerate(group_indices_tuples):
                        board[index] = 0
                    # already_checked_stones_indices.update(group_indices_tuples)
                    return induces_capture
        return induces_capture
